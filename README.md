...
`proxy` задаётся один раз и применяется ко всем источникам — флаг `--proxy` каждый раз
писать не нужно. Формат обычный: `socks5://127.0.0.1:1080` или `http://user:pass@host:port`.
...
Провайдеры выборочно режут CDN, с которых раздаётся видео, и список меняется со временем.
Раньше это выглядело так: mpv стартует и молча висит — ссылка получена, но до сервера
пакеты не доходят.

Теперь перед запуском плеера проверяется, отвечает ли сервер. Если нет — автоматически
берётся следующая озвучка, пока не найдётся рабочая. Если не открылась ни одна, выводится
сообщение вместо зависшего плеера.

Лечится прокси: пропишите его один раз в конфиге, дальше `ani` работает как обычно.
